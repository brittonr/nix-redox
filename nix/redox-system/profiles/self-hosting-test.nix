...
NIXEOF
                        ...
                        /nix/system/profile/bin/bash -c '
                          # Pre-create source tree that the builder will copy into $TMPDIR.
                          # Done here (in bash) because Ion cannot handle println!()
                          # syntax even inside single quotes.
                          mkdir -p /tmp/hello-cargo-src/src /tmp/hello-cargo-src/.cargo
                          cat > /tmp/hello-cargo-src/Cargo.toml << TOML
  [package]
  name = "hello"
  version = "0.1.0"
  edition = "2021"

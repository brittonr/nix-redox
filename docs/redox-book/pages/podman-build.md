...
Here are TL;DR instructions for **Podman Build** (recommended). [Native build is also available.
...
Create a new directory and run `podman_bootstrap.sh` inside of it. This will clone the repository and install **Podman**.
```
mkdir -p ~/tryredox
...
Navigate to the `redox` directory.
```
cd ~/tryredox/redox
...
Install Podman. If your Linux distribution is not supported, check the [installation instructions to determine which dependencies are needed. Or, run the following in your `redox` base` directory:
```

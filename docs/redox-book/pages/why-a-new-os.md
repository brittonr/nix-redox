...
Invalid type usage is verified at compile-time (NULLs can't cause undefined behavior, for example) and new types can be created to invalidate logic states that aren't allowed, which prevent/fix some logic bugs and reduce the formal verification effort
-
...
Beyond monolithic kernels being much more vulnerable to bugs there's also the much higher complexity of post-Internet modern hardware and drivers that didn't exist when monolithic kernels were adopted, but since the adoption of Internet and the implementation of the [TCP/IP network stack in [BSD Unix the complexity grew fast. This growth has caused an epidemic of bugs that are hard or invisible to debug due to all monolithic kernel components sharing the same memory address space.

...
guix shell --pure --container --emulate-fhs --network --share=$HOME \
  coreutils bash curl grep [email protected] nss-certs \
  -- bash -c 'curl --proto "=https" --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y --default-toolchain nightly'

guix shell --pure --container --emulate-fhs --network --share=$HOME \
  coreutils bash [email protected] nss-certs zlib glibc \
...
```
guix shell --pure --container --emulate-fhs --network --share=$HOME \
  coreutils bash curl wget [email protected] pkg-config fuse nss-certs zlib \
...
[email protected] [email protected] autoconf curl wget flex gperf expat \
gmp libpng jpeg sdl12-compat sdl2_ttf perl libtool m4 ninja \
meson [email protected] zip unzip llvm doxygen gpatch automake scons \

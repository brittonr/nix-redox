...
- `config/$ARCH/ci.toml` - The continuous integration variant, recipes added here become packages on the [build server. Aimed for packagers and developers.
- `config/$ARCH/jeremy.toml` - The build of [Jeremy Soller (creator/BDFL of Redox) with the recipes that he is testing at the moment.
### Build System Files

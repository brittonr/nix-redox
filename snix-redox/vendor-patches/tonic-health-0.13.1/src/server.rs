...
impl Health for HealthService {
    ...
    ) -> impl std::future::Future<Output = Result<Response<HealthCheckResponse>, Status>> + Send {
        async move {
    ...
    fn watch(
        &self,
        request: Request<HealthCheckRequest>,
    ) -> impl std::future::Future<Output = Result<Response<Self::WatchStream>, Status>> + Send {
        async move {
            let service_name = request.get_ref().service.as_str();

...
impl AutoCfg {
    ...
    pub fn with_dir<T: Into<PathBuf>>(dir: T) -> Result<Self, Error> {
        ...
        if !meta.is_dir() {
            return Err(error::from_str("output path is not a directory"));
        }

        // Redox/relibc can report directories as readonly even when the build
        // sandbox has granted write access. Cargo has already created OUT_DIR;
        // let the later probe writes be the source of truth instead of
        // rejecting the directory from permission metadata alone.

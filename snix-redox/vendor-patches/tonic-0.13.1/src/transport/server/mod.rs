        ...
        loop {
            ...
            let event = future::poll_fn(|cx| {
                if sig.as_mut().poll(cx).is_ready() {
                    return Poll::Ready(AcceptEvent::Signal);
                }

                if let Poll::Ready(io) = incoming.as_mut().poll_next(cx) {
                    return Poll::Ready(AcceptEvent::Io(io));
                }
    ...
    tokio::spawn(async move {
        {
            ...
            loop {
                ...
                let event = future::poll_fn(|cx| {
                    if let Poll::Ready(rv) = conn.as_mut().poll(cx) {
                        return Poll::Ready(ConnectionEvent::Done(rv));
                    }

                    if sleep.as_mut().poll(cx).is_ready() {
                        return Poll::Ready(ConnectionEvent::MaxAge);
                    }

                    if sig.as_mut().poll(cx).is_ready() {
                        return Poll::Ready(ConnectionEvent::Signal);
                    }
                ...
                match event {
                    ConnectionEvent::Done(rv) => {
                        if let Err(err) = rv {
                            debug!("failed serving connection: {:#}", err);
                        }
                        break;
                    }
                    ConnectionEvent::MaxAge => {
    ...
    F: Future,
{
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        match this.inner.as_mut() {
            Some(fut) => unsafe { Pin::new_unchecked(fut) }.poll(cx).map(|output| {
                this.inner = None;
                output
            }),
            None => Poll::Pending,

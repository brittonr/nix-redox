    ...
    IE: Into<crate::BoxError>,
{
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        let projected = unsafe { self.as_mut().get_unchecked_mut() };

        let Some(State(tls, tasks)) = projected.state.as_mut() else {
            return self.poll_next_without_tls(cx);
        };

        let inner = unsafe { Pin::new_unchecked(&mut projected.inner) };
        let select_output = ready!(pin!(select(inner, tasks)).poll(cx));
...
async fn select<IO: 'static, IE>(
    mut incoming: Pin<&mut impl Stream<Item = Result<IO, IE>>>,
    tasks: &mut JoinSet<Result<ServerIo<IO>, crate::BoxError>>,
) -> SelectOutput<IO>
where
    IE: Into<crate::BoxError>,
{
    let accept_future = tasks.join_next();
    let mut accept_future = pin!(accept_future);

    poll_fn(|cx| {
        match incoming.as_mut().poll_next(cx) {
            Poll::Ready(Some(Ok(stream))) => return Poll::Ready(SelectOutput::Incoming(stream)),
            Poll::Ready(Some(Err(e))) => return Poll::Ready(SelectOutput::TcpErr(e.into())),
            Poll::Ready(None) => return Poll::Ready(SelectOutput::Done),
            Poll::Pending => {}
        }

        if let Poll::Ready(accept) = accept_future.as_mut().poll(cx) {
            return Poll::Ready(match accept.expect("JoinSet should never end") {

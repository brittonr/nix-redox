    ...
    F: Future<Output = Result<http::Response<B>, E>>,
{
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        match &mut this.kind {
            Kind::Future(future) => unsafe { Pin::new_unchecked(future) }
                .poll(cx)
                .map_ok(|res| res.map(ResponseBody::wrap)),
            Kind::Status(status) => {
...
impl<B: http_body::Body> http_body::Body for ResponseBody<B> {
    ...
    fn poll_frame(
        ...
        let this = unsafe { self.get_unchecked_mut() };
        match &mut this.kind {
            ResponseBodyKind::Empty => Poll::Ready(None),
            ResponseBodyKind::Wrap(body) => unsafe { Pin::new_unchecked(body) }.poll_frame(cx),
        }
    }

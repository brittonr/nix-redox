    ...
    U: Stream<Item = Result<T::Item, Status>>,
{
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        // Manual projection so the Redox self-hosting build does not depend on
        // pin-project's proc-macro expansion in the guest compiler.
        let this = unsafe { self.get_unchecked_mut() };
        let mut source = unsafe { Pin::new_unchecked(&mut this.source) };
        let encoder = &mut this.encoder;
        let compression_encoding = &mut this.compression_encoding;
        let max_message_size = &mut this.max_message_size;
        let buf = &mut this.buf;
        let uncompression_buf = &mut this.uncompression_buf;
        let error = &mut this.error;
        let buffer_settings = encoder.buffer_settings();
    ...
    U: Stream<Item = Result<T::Item, Status>>,
{
    ...
    fn poll_frame(
        ...
        // Manual projection; see EncodedBytes::poll_next above.
        let this = unsafe { self.get_unchecked_mut() };

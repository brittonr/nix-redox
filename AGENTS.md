...
- Focused `kernel-rebuild-test` (2026-04-14): 10 pass, 0 fail out of 10 tests; durable capture `/var/tmp/redox-self-hosting-captures/20260414T230502-kernel-rebuild-test/`; this proves guest-native kernel + bootloader store artifacts, manifest/GC-root staging, and `snix system boot` + `activate-boot` smoke for selecting the guest-produced kernel path
